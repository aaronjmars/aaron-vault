...
export class TileField {
  ...
  resize() {
    ...
    const xs: number[] = [], ys: number[] = [], rw: number[] = [],
      sp: number[] = [], sd: number[] = [], hs: number[] = [];
  ...
  private frame = (t: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (colorAmt > 0.04) {
        // quantize (hue, alpha) into buckets to keep fill() calls low
        let hue: number;
        let chroma: number;
        if (ROWS[rowOf[i]].bright) {
          // party rows: full-hue wander
          hue = (hueSeed[i] * 360 + time * 26) % 360;
          chroma = 0.19;
        } else {
          // brand rows: hue drifts around the brand hue with the flow
