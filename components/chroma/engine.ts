...
export class ChromaGlow {
  ...
  private cyclePhaseT = 0;
  private holdDur = 1.15;
  private outDur = 0.24;
  private inDur = 0.32;
  private disperse = 0;
  ...
  private tweenedPalette(): ChromaParams {
    ...
    return {
      ...
      // step the boolean halfway through
  ...
  private frame(now: number) {
    ...
    this.fx += (ax - this.fx) * 0.2;
    this.fy += (ay - this.fy) * 0.2;

    if (this.cycle) {
      // word cycle: hold -> disperse out -> swap -> settle in
  ...
  private stepCycle(dt: number) {
    ...
    if (this.cyclePhase === "hold") {
      ...
      if (this.cyclePhaseT >= this.holdDur) {
        // start the exit: palette tweens half out during disperse
      ...
      this.disperse = easeIn(k);
      this.palMix = 0.5 * k;
      if (k >= 1) {
        // swap the word under full disperse (invisible seam)
        if (this.nextPending) {
