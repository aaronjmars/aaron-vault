...
export class DesignTiles {
  ...
  constructor(host: HTMLElement) {
    ...
    WORDS.forEach((word, i) => {
      ...
      Object.assign(outer.style, {
        ...
        // grid-template-columns 0fr -> 1fr wipes the tile open without scaling
  ...
  private layout() {
    for (const tile of this.tiles) {
      ...
      tile.rects = [];
      if (!m) {
        // measurement failed (no ink-box support): one plain rect
  ...
  private buildRects(tile: Tile, m: WordMetrics) {
    ...
    // glyphs clip into a common band so every tile has the same height
    ...
    // auto width from the aspect ratio
  ...
  private bindEvents() {
    this.tiles.forEach((tile) => {
      // entering a tile re-rolls it immediately
