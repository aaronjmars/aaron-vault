...
void main() {
  ...
  if (abs(uSyQ.y) + abs(uSyQ.z) < 1e-5) {
    ...
    for (int i = 0; i < SAMPLES; i++) {
      ...
      vec3 uk = us * uK;
      vec3 syv = uSyQ.x + uSyQ.y * uk + uSyQ.z * uk * uk;
      acc.r += w * tap(
        syv.r * (uk.r < uSwapU ? uSwapScl.x : uSwapScl.y),
        uk.r < uSwapU ? uHalfA : uHalfB, W, front);
      acc.g += w * tap(
        syv.g * (uk.g < uSwapU ? uSwapScl.x : uSwapScl.y),
        uk.g < uSwapU ? uHalfA : uHalfB, W, front);
      acc.b += w * tap(
        syv.b * (uk.b < uSwapU ? uSwapScl.x : uSwapScl.y),
